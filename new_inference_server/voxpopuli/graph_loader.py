...
def _record_to_graph(record: dict[str, Any]) -> GraphSample:
...
def build_utterance_graph(
    ...
    if file_name is not None:
        ...
        with record_path.open("r", encoding="utf-8") as file:
            record = json.load(file)
        return _record_to_graph(record)
    ...
    return _record_to_graph(records[0])


def load_speaker_graphs(data_root: str | Path, speaker_id: str) -> list[GraphSample]:
...
def build_utterance_graphs(
    ...
    for speaker_id in speaker_ids:
        graphs.extend(load_speaker_graphs(data_root, speaker_id))
    return graphs
